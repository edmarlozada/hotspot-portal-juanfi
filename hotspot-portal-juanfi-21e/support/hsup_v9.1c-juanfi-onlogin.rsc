...
local cfgRandoMac 1 ;# Ramdom Mac Sync Fix
...
if (!($eMail~"active")) do={
  ...
  local iMacName [$eDiscard $iDevMac ":"]
  local iActMail ("$iMacName@juanfi.$iVer1.active")
  local iRoot    [ip hotspot profile get [.. get [find interface=$iDevInt] profile] html-directory]
  local cType    "EXT"; if ([system scheduler find name=$iUsr]="") do={ set cType "NEW" }
  ...
  $eLogDebug ("JuanFi-$cType => user=[ $iUsr ] ip=[ $iDevIP ] mac=[ $iDevMac ] interface=[ $iDevInt ] comment=[ $iComment ]") $cfgShowLogs
  local iActiveT "V"
  local iDataLoc "xData"
  local iUsrFile "VU$iUsr"
  local iMacFile "VM$iMacName"
  ...
  local iEvent ("# JuanFi $iVer1 $iVer2 #\r\n".\
    ...
    set iEvent ("$iEvent\r\n".\
                ...
                "}\r\n".\
                "file remove [find name=\"$iRoot/data/$iMacName.txt\"]\r\n")
    set iEvent ("$iEvent".\
                "file remove [find name=\"$iRoot/$iDataLoc/$iUsrFile.txt\"]\r\n".\
                "file remove [find name=\"$iRoot/$iDataLoc/$iMacFile.txt\"]\r\n")
  ...
  if ($cfgAddFiles) do={
    local eSaveData do={
      ...
      if ([file find name="$iRoot/$iPath/$iFile.txt"]="") do={
        file print file="$iRoot/$iPath/$iFile.txt" where name="$iFile.txt"; delay 1s
    ...
    $eSaveData $iUsr $iRoot "data"  ("$iMacName")  ("$iUsr#$iUserExp")
    # Add Data for Edmar-Portal
    local iDataVer "v9.1"
    local iUsrPass [ip hotspot user get [find name=$iUsr] password]
    $eSaveData $iUsr $iRoot $iDataLoc ("$iUsrFile") ("$iDevMac,$iSaleAmt,$iUsrTime,$iValidty,$iUserBeg,$iUserExp,$iActiveT,$iDataVer")
    $eSaveData $iUsr $iRoot $iDataLoc ("$iMacFile") ("$iUsr,$iUsrPass,$iActiveT,$iDataVer")
  }
  ...
  if ($cfgTelegram) do={
    ...
    local iType "new"; if ($cType="EXT") do={ set iType "ext" }
  ...
  $eLogDebug ("JuanFi-IN => user=[ $iUsr ] ip=[ $address ] mac=[ $"mac-address" ] email=[ $eMail ] login-by=[ $($aHSA->"login-by") ]") $cfgShowLogs
}
